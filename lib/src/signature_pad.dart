...
abstract class SignaturePadBase {
  ...
  Duration get throttleDuration => Duration(milliseconds: opts.throttle);
  ...
  _CurveWidth _addMark(Mark p) {
    ...
    if (points.length > 2) {
      ...
      var c3 = tmp.t1;
      var curve = Bezier(points[1], c2, c3, points[2]);
      var widths = _calculateCurveWidths(curve);

      // Remove the first element from the list,
      // so that we always have no more than 4 points in points array.
      points.removeAt(0);
      return _CurveWidth(curve, widths);
    }
    return null;
  }
  ...
  _Tuple<Point<double>> _calculateCurveControlPoints(
    ...
    var m1 = Point((s1.x + s2.x) / 2.0, (s1.y + s2.y) / 2.0);
    var m2 = Point((s2.x + s3.x) / 2.0, (s2.y + s3.y) / 2.0);
    ...
    assert(l2 is double);
    var k = l2 / (l1 + l2);
    var cm = Point(m2.x + (dxm * k), m2.y + (dym * k));

    var tx = s2.x - cm.x;
    var ty = s2.y - cm.y;

    return _Tuple<Point<double>>(
        Point(m1.x + tx, m1.y + ty), Point(m2.x + tx, m2.y + ty));
  }
  ...
  _Tuple<double> _calculateCurveWidths(Bezier curve) {
    ...
    var widths = _Tuple(_lastWidth, newWidth);
  ...
  void drawDot(Mark point) {
    var width = dotSize;
    drawPoint(point.x, point.y, width);
  }
